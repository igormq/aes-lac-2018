...
class VoxForge(Corpus):
    ...
    DATASET_URLS = {"train": ["https://www.dropbox.com/s/wrguetal6xmrgta/voxforge-ptbr.tar.gz?dl=1"]}

    def __init__(self, target_dir='voxforge_dataset', min_duration=1, max_duration=15, fs=16e3, name='voxforge'):
        super().__init__(
            VoxForge.DATASET_URLS, target_dir, min_duration=min_duration, max_duration=max_duration, fs=fs, name=name)

    def get_data(self, root_dir, set_type):
        audio_paths = list(self.find_audios(root_dir))

        search_dirs = set([os.path.split(p)[0].replace('{}wav'.format(os.path.sep), '') for p in audio_paths])

        data = []
        for curr_dir in search_dirs:
            transcriptions_file = glob.glob(os.path.join(curr_dir, '**', 'PROMPTS'), recursive=True)[0]

            assert os.path.isfile((transcriptions_file)), "prompts.txt not found in {}".format(root_dir)
            ...
            with open(transcriptions_file, 'r', encoding='utf8') as f:
                for line in f.readlines():
                    ...
                    if len(filtered_audio_paths) != 1:
                        print(
                            ('WARNING: Found zero or more than one audio file for the transcription id {} in {}.'
                             ' Skipping...').
